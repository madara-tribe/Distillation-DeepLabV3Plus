...
from losses import DistillationLoss, MultiClassCriterion
...
class Solver:
    ...
    def train(self, config, smodel, device, num_workers, pin_memory):
        lr = config.lr
        enable_vis = False
        vis_env = 'main'
        ...
        if separable_conv and 'plus' in 'deeplabv3plus_mobilenet':
            convert_to_separable_conv(smodel.classifier)
        set_bn_momentum(smodel.backbone, momentum=0.01)
        metrics = StreamSegMetrics(num_classes)

        print('Set up optimizer and model......')
        optimizer, scheduler = create_optimizer(smodel, config)
        ...
        while True: # cur_itrs < config.total_itrs:
            ...
            for (images, labels) in train_loader:
                ...
                lesson = self.tmodel(images)
                outputs = smodel(images)
                label_loss = self.criterion(outputs, labels)
                diss_loss = self.distillation_loss(outputs, lesson)
                ...
                if (cur_itrs) % val_interval == 0:
                    self.save_ckpt('checkpoints/latest_deeplabv3plus_os{}d.pth'.format(cur_itrs), cur_itrs, optimizer, scheduler, best_score, smodel)
                    print("validation...")
                    smodel.eval()
                    val_score, ret_samples, interval_valloss = self.validate(
                        model=smodel, loader=val_loader, device=device, metrics=metrics, ret_samples_ids=vis_sample_id)
                    print(metrics.to_str(val_score))

                    if val_score['Mean IoU'] > best_score:  # save best model
                        best_score = val_score['Mean IoU']
                        self.save_ckpt('checkpoints/best_os%d.pth' %
                                (output_stride), cur_itrs, optimizer, scheduler, best_score, smodel)
